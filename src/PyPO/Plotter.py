...
def plotBeam2D(plotObject, field, contour,
    ...
    if not amp_only:
        ...
        if scale == Scales.LIN:
            ...
            phasefig = ax[1].pcolormesh(grid_x1 / units, grid_x2 / units, np.angle(field), cmap=cmaps.parula, shading='auto')
        ...
        if scale == Scales.LIN:
            ...
            if contour is not None:
                cont = ax.contour(grid_x1 / units, grid_x2 / units, contour_dB, levels, cmap=cm.binary, linewidths=0.5)
                ax.clabel(cont)
...
def plotBeamCut(x_cut, y_cut, x_strip, y_strip, vmin, vmax, units):
    ...
    ax.set_xlim(np.min(x_strip / units), np.max(x_strip / units))
